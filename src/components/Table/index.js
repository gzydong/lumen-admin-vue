...
export default {
  ...
  created () {
    this.localPagination = ['auto', true].includes(this.showPagination) && Object.assign({}, this.localPagination, {
      ...
      showQuickJumper:this.showPageJump
    }) || false
    ...
    loadData (pagination, filters, sorter) {
      ...
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          ...
          // 这里用于判断接口是否有返回 r.total 且 this.showPagination = true 且 page 和 page_size 存在 且 totalCount 小于等于 page * page_size 的大小
          // 当情况满足时，表示数据不满足分页大小，关闭 table 分页功能
          try {
            if ((['auto', true].includes(this.showPagination) && r.total <= (r.page * this.localPagination.page_size))) {
